...
public:
	...
	inline void AddPoint(const std::pair<Vec, T> &point, real radius) {
		hitPoints.push_back(point);
		radii.push_back(radius);
		hpbbox.Fit(point.first);
	}

	inline void AddPoint(std::pair<Vec, T> &&point, real radius) {
		hitPoints.push_back(point);
		radii.push_back(radius);
		hpbbox.Fit(point.first);
	}
	...
	void BuildHashGrid(real maxRadius = 0.8) {
		...
		ParallelFor(0, hitPointNum, [&](int i) {
			std::pair<Vec, T> &hp = hitPoints[i];
				Vec BMin = ((hp.first - radii[i]) - hpbbox.minPoint) * invHashCellSize;
				Vec BMax = ((hp.first + radii[i]) - hpbbox.minPoint) * invHashCellSize;
